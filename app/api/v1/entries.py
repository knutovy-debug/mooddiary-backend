...
async def get_today_count(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(func.count(Entry.id)).where(Entry.user_id == current_user.id, Entry.created_at >= start_of_day))
...
async def create_entry(payload: dict, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if not current_user.is_subscribed:
        start_of_day = get_moscow_now().replace(hour=0, minute=0, second=0, microsecond=0)
        count_result = await db.execute(select(func.count(Entry.id)).where(Entry.user_id == current_user.id, Entry.created_at >= start_of_day))
        if (count_result.scalar() or 0) >= 3: raise HTTPException(status_code=403, detail="Бесплатный лимит исчерпан. Оплатите подписку.")
    try:
